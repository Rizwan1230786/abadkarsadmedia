...
class WebpagesController extends Controller
{
    ...
    public function subpageslisting(Request $request){
        $subpage=subpages::where('parent_id',$request->id)->get();
        return view('admin.modules.webpages.subpageslisting',compact('subpage'));
    }
    public function subpages_create(Request $request)
    {
        $webpages=Webpages::all();
        $data = null;
        $data['updateId'] = $updateId = ($request->id ?? 0);
        if (is_numeric($updateId) && $updateId > 0) {
            $data['record'] = subpages::where('id', $updateId)->first();
        }
        return view('admin.modules.webpages.subpagescreate', compact('data','webpages'));
    }
    public function submitSubpages(Request $request) {
        // "website_title" => $request->website_title,
        $type = 'error';
        $validator = Validator::make($request->all(), [
            'url_slug' => ['required',"max:150","min:1"],
        ...
        if ($validator->passes()) {
            ...
            $updateId = $request->id;
                $data = array("page_title" => $request->page_title,"parent_id" => $request->parent_id, "head_title" => $request->head_title, "meta_title" =>$request->meta_title, "meta_description" => $request->meta_description, "meta_keywords" => $request->meta_keywords, "short_description" => $request->short_description, "url_slug" => $request->url_slug, "page_content" => $request->page_content, "page_rank" => ($request->page_rank ?? 0));
                if (isset($updateId) && !empty($updateId) && $updateId > 0) {
                    $data['id'] = $updateId;
                    $message = "subpages update successfully";
                }
                subpages::updateOrCreate(array('id' => $updateId), $data);
            } else {
                $message = $validator->errors()->toArray();
            }
            return response()->json(['type' => $type, 'message' => $message]);
    }
    public function update_status_subpages(Request $request){
        $userid = $request->id;
        $status = $request->status;
        if ($status == 1) {
            $status = 0;
          ...
          $status = subpages::whereId($userid)->update(array('is_publish' => $status));
            if (isset($status) && !empty($status)) {
                $type = "success";
                $message = "Status updated successfully";
            }
