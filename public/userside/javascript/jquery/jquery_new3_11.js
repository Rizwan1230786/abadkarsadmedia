...
(function($) {
    $.extend($.fn, {
        livequery: function(b, c, d) {
            var e = this,
                q;
            if ($.isFunction(b)) d = c, c = b, b = undefined;
            $.each($.livequery.queries, function(i, a) {
                if (e.selector == a.selector && e.context == a.context && b == a.type && (!c || c.$lqguid == a.fn.$lqguid) && (!d || d.$lqguid == a.fn2.$lqguid)) return (q = a) && false
            });
            q = q || new $.livequery(this.selector, this.context, b, c, d);
            q.stopped = false;
            q.run();
            return this
        },
        expire: function(b, c, d) {
            ...
            if ($.isFunction(b)) d = c, c = b, b = undefined;
            $.each($.livequery.queries, function(i, a) {
                if (e.selector == a.selector && e.context == a.context && (!b || b == a.type) && (!c || c.$lqguid == a.fn.$lqguid) && (!d || d.$lqguid == a.fn2.$lqguid) && !this.stopped) $.livequery.stop(a.id)
            });
            return this
        }
    });
    $.livequery = function(a, b, c, d, e) {
        this.selector = a;
        ...
        d.$lqguid = d.$lqguid || $.livequery.guid++;
        if (e) e.$lqguid = e.$lqguid || $.livequery.guid++;
        return this
    };
    $.livequery.prototype = {
        stop: function() {
...
(function($) {
    $.fn.ajaxSubmit = function(o) {
        ...
        if (p.veto) return this;
        var a = this.formToArray(o.semantic);
        if (o.data) {
            for (var n in o.data) a.push({
                name: n,
                value: o.data[n]
            })
        }
        if (o.beforeSubmit && o.beforeSubmit(a, this, o) === false) return this;
        ...
        var r = this,
            callbacks = [];
        if (o.resetForm) callbacks.push(function() {
            r.resetForm()
        });
        if (o.clearForm) callbacks.push(function() {
            r.clearForm()
        });
        if (!o.dataType && o.target) {
            var u = o.success || function() {};
            callbacks.push(function(a) {
                if (this.evalScripts) $(o.target).attr("innerHTML", a).evalScripts().each(u, arguments);
                else $(o.target).html(a).each(u, arguments)
            })
        } else if (o.success) callbacks.push(o.success);
        o.success = function(a, b) {
            for (var i = 0, max = callbacks.length; i < max; i++) callbacks[i](a, b, r)
        };
        var v = $('input:file', this).fieldValue();
        var w = false;
        for (var j = 0; j < v.length; j++)
            if (v[j]) w = true;
        if (o.iframe || w) fileUpload();
        else $.ajax(o);
        ...
        function fileUpload() {
            ...
            if (g) $.event.trigger("ajaxSend", [l, f]);
            var m = 0;
            var n = 0;
            setTimeout(function() {
                i.appendTo('body');
            ...
            function cb() {
                ...
                try {
                    ...
                    if (f.dataType == 'json' || f.dataType == 'script') {
                        ...
                        if (f.dataType == 'json') eval("data = " + b);
                        else $.globalEval(b)
                    } else if (f.dataType == 'xml') {
                        b = l.responseXML;
                        if (!b && l.responseText != null) b = toXml(l.responseText)
                    } else {
                        b = l.responseText
                    }
                } catch (e) {
                ...
                if (a) {
                    f.success(b, 'success');
                    if (g) $.event.trigger("ajaxSuccess", [l, f])
                }
                if (g) $.event.trigger("ajaxComplete", [l, f]);
                if (g && !--$.active) $.event.trigger("ajaxStop");
                if (f.complete) f.complete(l, a ? 'success' : 'error');
                setTimeout(function() {
                    i.remove();
                    l.responseXML = null
                }, 100)
...
$(document).ready(function() {
    d_name_h = window.location.href;
    if (d_name_h.indexOf("http://wahdo.com") > -1) {
        I("frmlogin").onsubmit = function() {
            ab = randomstring();
...
function empty(param) {
    if (typeof(param) == 'object') return empty(param.value)
    return (param == "" || param == 0);
}

function split_string(str, chr) {
    chr = chr || ",";
    str = str + "";
    var output = [];
    if (str != "" && str != "0") {
...
(function($) {
    $.fn.extend({
        center_screen: function(height) {
            var win_height = $(window).height();
            var ypos = $(window).scrollTop();
            var xpos = window.innerWidth != null ? window.innerWidth : document.documentElement && document.documentElement.clientWidth ? document.documentElement.clientWidth : document.body != null ? document.body.clientWidth : null;
            var bounds = {
                w: $(this).width(),
                h: $(this).height()
            };
            if (height) bounds.h = height;
            xpos = (xpos - bounds.w) / 2;
...
//var loading_html = "<center><img src='"+zn_loading+"' style='padding:10px;' /></center>";
var loading_small = this_domain + "/images/common/loading_small.gif";
var timeoutid = 0;

function bg_filter() {
    var filter = I('backgroundFilter');
    filter.style.height = I("footer_bar").offsetTop + "px";
    filter.style.display = 'block';
    filter.onclick = close_popup_window;
    return filter;
}
var popup_params = 0;

function close_popup_window() {
    I("popup_window").style.display = "none";
    I("backgroundFilter").style.display = "none";
    I("backgroundFilter").onclick = null;
    if (popup_params.onclose) eval(popup_params.onclose);
}

function popup_window(options) {
    popup_params = options;
    bg_filter();
    popup = I("popup_window");
    className = (popup_params.size == "small") ? "popup_round small" : "popup_round";
    $(popup).attr({
        'class': className
    }).children(".mid_part").html(loading_html);
    $(popup).show().center_screen();
    $.get(popup_params.url, popup_params.params, function(str) {
        $(popup).html(str).center_screen();
        if (popup_params.afterAjax) eval(popup_params.afterAjax);
        $(popup).find("input[type=text]")[0].focus();
    });
}

function showHeaderSettings() {
    popup_window({
        url: this_domain + "/includes/header_settings_div.php",
        size: "small"
    });
}

function HideTabDropdoun() {
    if (I('tabs_tool')) {
        clearTimeout(timeoutid);
        timeoutid = setTimeout("$('#tabs_tool').hide();", 400)
    }
}

function ShowTabDropdown(a) {
    if (I('tabs_tool')) {
        $('#tabs_tool').show();
        clearTimeout(timeoutid);
    }
...
$("#tabs_search_go").live("click", function(a) {
...
function showPriceText(event) {
    ...
    if (event) {
        ...
        for (k = 0; k < inps.length; k++) {
            if (inps[k].getAttribute("name") == "prices")
                pricetag = inps[k];
        }
    }
    if (pricetag == undefined) return;
    if (isNaN(pricetag.value)) {
        showprice = "<font color='red'>Price must contain numbers only</font>";
    } else {
        showprice = getPriceText(pricetag.value);
    }
    if (I("stext_facebox") != null && I("stext_facebox").style.display == "none")
...
function email_alert_page() {
    var params = {
        cat_id: $("#loc_cat_id").val(),
        subtype: $("#subtype").val(),
        purpose: $("#purpose").val(),
        type: $("#type").val()
    };
    url = this_domain + "/body/common/listings_email_alerts.php";
    popup_window({
        url: url,
        onclose: '$("#jqac_id").remove();',
        params: params
    });
}
...
function price_convert(price_text) {
    if (function_exists("overlib"))
        return overlib(price_text, CAPTION, "Price Information", WIDTH, 260);
}
...
function land_convert(area_sqft) {
    ...
    if (function_exists("overlib"))
        return overlib(area_text, CAPTION, "Covered Area", WIDTH, 180);
}

function land_convert_txt(obj, area_sqft) {
    area_text = number_format(parseInt(area_sqft), 0, ".", ",") + " Square Feet<br/>";
    sq_mt = number_format(parseInt(area_sqft) * 0.09290304, 0, ".", ","); ////Sq Meter
    area_text += sq_mt + " Square Meters<br/>";
    sq_yd = number_format(parseInt(area_sqft) * 0.111111111, 0, ".", ","); ////Sq Yard   
    area_text += sq_yd + " Square Yards<br/>";
    showtooltip(obj, area_text, 'Covered Area', 180)
    //return area_text;
}
...
function community_events() {
    I("newQues").onsubmit = function() {
        var def_text = I("comm_ask_default_text").value;
        var curr_text = I("thread_title").value;
        if (curr_text == def_text) {
            I("thread_title").value = "";
        }
    };
    I("thread_title").onblur = function() {
        var def_text = I("comm_ask_default_text").value;
        var curr_text = I("thread_title").value;
        if (curr_text == "") {
            I("thread_title").value = def_text;
            I("thread_title").style.color = "#999999";
        }
    };
    I("thread_title").onclick = function() {
        var def_text = I("comm_ask_default_text").value;
        var curr_text = I("thread_title").value;
        if (def_text == curr_text) {
            I("thread_title").value = "";
            I("thread_title").style.color = "#000000";
        }
    };
}

function orra_contact_page(page_name) {
    popup_div = $("#info_email_div");
    if (popup_div.css("display") == "block") {
        filter.onclick();
    ...
    $(popup_div).html(loading_html).show().center_screen();;
    if (typeof page_name === "undefined") page_name = "orra_marina";
    $.get(this_domain + "/images/ads_banners/" + page_name + ".php", function(str) {
        $(popup_div).html(str).center_screen();
    });
}
...
function overhere(e, x, y) {
    ...
    if (initialize) {
        ...
        MoveToolTip(truebody().scrollLeft + e.clientX + x, scroll_top + e.clientY + y);
    }
}
...
function EnterContent(layerName, TTitle, TContent, width) {
    if (!width) width = 250;
    ContentInfo = '' +
        '<table class="facebox" style="width:' + width + 'px;"> ' +
        '<tr><td class="facebox_tl"><td class="facebox_b"><td class="facebox_tr">' +
        '<tr><td class="facebox_b">' +
        '<td class="facebox_body">' +
        '<div class="tooltiptitle">' + TTitle + '</div>' +
        '<div style=\"background-color:' + subColor + '\" id="tooltipcontent">' + TContent + '</div>' +
        "<td class='facebox_b'>" +
        '<tr><td class="facebox_bl"><td class="facebox_b"><td class="facebox_br">' +
        '</table>';
    //ReplaceContent();
    ToolTip.innerHTML = ContentInfo;
}
...
function show_hide_tab_div() {
    type = $('#tab_type').val();
    purpose = $('#tab_purpose').val();
    if (type == 2) {
        I("tab_beds_ajax_container").style.display = "none";
    } else {
        I("tab_beds_ajax_container").style.display = "block";
    }
    if (purpose == 3) {
        I("wantedfor_div").style.display = "block";
        $(".pointing-arrow")[1].style.display = "block";
        I("tab_beds_ajax_container").style.display = "none";
    } else if (purpose == 'oh') {
        I("ohfor_div").style.display = "block";
        $(".pointing-arrow")[1].style.display = "block";
        I("tab_beds_ajax_container").style.display = "none";
    } else {
        I("wantedfor_div").style.display = "none";
        I("ohfor_div").style.display = "none";
        $(".pointing-arrow")[1].style.display = "none";
        I("tab_beds_ajax_container").style.display = "block";
    }
}

function tab_purpose_change() {
    purpose = $('#tab_purpose').val();
    show_tab_more_option();
    return false;
    if (purpose == 3) {
        show_tab_more_option();
        $('#wantedfor_div').css({
        ...
        $(".pointing-arrow")[1].style.display = "block";
        if (I("tab_beds_ajax_container"))
            I("tab_beds_ajax_container").style.display = "none";
    } else if ($('#tab_search_container').html() != "") {
        $('#wantedfor_div').css({
            "display": "none"
        });
        $(".pointing-arrow")[1].style.display = "none";
...
function show_tab_more_option(ajax_param) {
    type = parseInt($('#tab_type').val());
    purpose = $('#tab_purpose').val();
    main_div_display = $('#tab_search_container').css("display");
    //if(main_div_display == "none")
    {
        load_image = "<img src='" + loading_small + "' />";
        //if($('#tab_search_container').html() == "")
        {
            param = {
                type: type,
                purpose: purpose,
            ...
            if (ajax_param != null)
                param = ajax_param;
            $('#tab_search_container').html(load_image);
            $.get(this_domain + "/profolio/includes/tabs_search_ajax.php", param,
                function(str) {
                    I('tab_search_container').innerHTML = str;
                    show_hide_tab_div();
                });
            $(".pointing-arrow")[0].style.display = "block";
        }
        $('#tab_search_container').css({
            "display": "block"
        });
    }
}
$("#tab_wanted_for").live("change", function() {
    if (this.value == "Rent") purpose = 2;
    else purpose = 1;
    I('tab_price_div').innerHTML = I('tab_price_' + purpose).innerHTML;
});
$("#tab_type").live("change", function() {
    type = $(this).val(); //alert(type);
    //if(type >= 0 && type != "") show_tab_more_option();
    main_div_display = $('#tab_search_container').css("display");
    if (main_div_display == "none")
        show_tab_more_option();
    else if (I('tab_subtype_ajax_container') != null) {
        $.get(this_domain + "/profolio/includes/tabs_search_ajax.php", {
            ptype: type
        }, function(str) {
            document.getElementById('tab_subtype_ajax_container').innerHTML = str;
            show_hide_tab_div();
        });
    }
});

$("#tab_subtype").live("change", function() {
    subtype_value = this.value;
    if ((I("tab_purpose").value != 3 && I("tab_purpose").value != 'oh') && (subtype_value == 3 || subtype_value == 4 || subtype_value == 5 || subtype_value == 9 || subtype_value == 11 || subtype_value == 16))
        I("tab_beds_ajax_container").style.display = "block";
    else
        I("tab_beds_ajax_container").style.display = "none";
...
/* tooltip plugin code start */
var current_tooltip_object = 0;
var current_tooltip = 0;
var tooltip_timeout_id = 0;
var tooltip_object_rect = 0;

function object_rect(obj_param) {
    var x = 0,
        y = 0,
        w = 0,
        h = 0,
    ...
    w = obj_param.clientWidth;
    h = obj_param.clientHeight;
    xx = x + w;
    yy = y + h;
    return {
        x: x,
...
function tooltip_hide() {
    ...
    tooltip_timeout_id = setTimeout(function() {
        current_tooltip[0].style.display = "none";
    }, 400);
}

function tooltip_show() {
    current_tooltip[0].style.display = "block";
    clearTimeout(tooltip_timeout_id);
}

function tooltip_move(scrollTop_div) {
    clearTimeout(tooltip_timeout_id);
    tooltip_object_rect = object_rect(current_tooltip_object);
    fixed_parent = 0;
    obj_temp = current_tooltip_object;
    while (obj_temp && obj_temp.tagName != "BODY") {
        if ($(obj_temp).css("position") == "fixed") fixed_parent = 1;
        obj_temp = obj_temp.parentNode;
    }
    if ($(scrollTop_div).length) //fixed height container that can be scrolled
    {
        tooltip_object_rect.y = tooltip_object_rect.y - $(scrollTop_div).scrollTop();
    } else if (fixed_parent) {
        tooltip_object_rect.y += $(window).scrollTop(); //for tooltip on map
    }
    var link_width = $(current_tooltip_object).width();
    var offset_top = current_tooltip.height() - 6;
    var offset_left = current_tooltip.width() - 32 - link_width;
    $(current_tooltip).css({
        top: tooltip_object_rect.y - offset_top,
        left: tooltip_object_rect.x - offset_left
    });
}

function showtooltip(obj, tooltip_txt, caption_txt, width, scrollTop_div) {
    if (!width) width = 275;
    inner_div_width = width - 32;
    current_tooltip_object = obj;
    if (!current_tooltip) current_tooltip = $("#over_lib");
    if (caption_txt) caption_txt = "<font class='tooltip_caption'>" + caption_txt + "</font><br />";
    else caption_txt = "";
    current_tooltip[0].style.width = width + "px";
    I("over_lib_hm").style.width = inner_div_width + "px";
    I("over_lib_fm").style.width = inner_div_width + "px";
    I("over_lib_data").innerHTML = caption_txt + tooltip_txt;
    tooltip_move(scrollTop_div);
    tooltip_show();
    if (current_tooltip_object.onmouseout == undefined)
        current_tooltip_object.onmouseout = tooltip_hide;
}
/* tooltip plugin code end */

cat_combo_url = this_domain + '/gmaps/ajax_caegory_list.php';
$.get(cat_combo_url, {}, function(obj) {
    cat_json_obj = eval(obj);
});


function ajax_logout() {
    log_url = this_domain + "/index.php?ajax_logout";
    $.get(log_url, {}, function(str) {
        document.location = this_domain + '/';
    });
    return false;
}

$("#header_logout").livequery("click", function() {
    ajax_logout();
    return false;
});


function resize_pointing_arrow() {
    var height = $("#tab_search_main").height() - 27;
    if (I("tab_cat_id").value.replace(",", "") == "") height = 4;
    $(".pointing-arrow .m").height(height);
}

setTimeout("$('#search_icon').fadeOut(3000, function () {})", 7000);
...
var current = 0;
var imageWidth = 2318;
var headerWidth = imageWidth;
var restartPosition = -(imageWidth - headerWidth);
var drestartPosition = -(1440 - 1440);
...
function scrollbg() {
    ...
    $('#footer_top').css("background-position", current + "px 0");
}
var dscrollSpeed = 25;
var dcurrent = 0;

function scrollbg_rite() {
    dcurrent -= step;
    if (dcurrent == drestartPosition) {
        dcurrent = 0;
    }
    $('#footer_bottom').css("background-position", dcurrent + "px 0");
}

window_self_href = window.location.href;

if (window_self_href.indexOf("profolio") < 0) {
    $(document).ready(function() {
        if (I('css_for_safari') != undefined) {
            if (navigator.userAgent.indexOf('Safari') != -1 && navigator.userAgent.indexOf('Chrome') == -1)
                I('css_for_safari').disabled = false;
            else
                I('css_for_safari').disabled = true;
        }
        var init = setInterval("scrollbg()", scrollSpeed);
        var dinit = setInterval("scrollbg_rite()", dscrollSpeed);
    });
}

$(".auto_textbox").live("focus", function() {
    this.value = "";
});

$(".auto_textbox").live("blur", function() {
    if (!this.value) this.value = $(this).attr("defvalue");
});

$(".show_hide_soc_btns").live("click", function() {
    var disp = $("#social_buttons").css("display");
    url = this_domain + "/body/common/social_buttons_ajax.php?display=" + disp;
    $.get(url, function(str) {
        eval(str);
    });
});
/* cat_box script start*/
var cat_box = {
    output: 0,
    input: 0,
    multi: 0
};
mouse_in_suggestion_menu = false;


$('input.categoryfilter').live('focus', function() {
    cat_box.focus = 1;
    cat_box.input = this; //category input field
    cat_box.input.value = "";
    cat_box.output = I(this.id.replace("_input", "")); //category hidden value
    cat_box.multi = parseInt($(this).attr("multi"));
    cat_box.container_id = $(this).attr("container_id");
});
$('input.categoryfilter').live('blur', function() {
    if (!mouse_in_suggestion_menu) {
        cat_box.focus = 0;
        setTimeout('$(".jqac-menu").remove()', 100);
        def_values = $(this).attr("dvalue").split(",");
        if (cat_box.multi && cat_box.output.value != "" && cat_box.output.value != 0)
            this.value = def_values[1];
        else
            this.value = def_values[0];
    }
});
$('input.categoryfilter').livequery(function(event) {
    $(this).keydown(function(e) {
        if (e.which == 8 && this.value == "") $(this).prev("a").click();
    });
    call_back = eval($(this).attr('call_back'));
    ajax_get = eval($(this).attr('ajax_get'));
    $(this).autocomplete({
        ajax_get: ajax_get,
        callback: call_back,
        multi: true
...
function remove_category(cat_link, cat_id) {
    var inp = $(cat_link).remove().attr("inp");
    cat_box.output = I(inp);
    cat_box.input = I(inp + "_input");
    cat_box.multi = parseInt($(cat_box.input).attr("multi"));
    cat_box.container_id = $(cat_box.input).attr("container_id");
    if (cat_box.multi)
        cat_box.output.value = cat_box.output.value.replace(cat_id + ",", "");
    else {
        $(cat_box.input).val("");
        $(cat_box.output).val("");
    }
    $(cat_box.input).show().focus();
    update_category();
}

function empty_category(input) {
    cat_box.output = I(input);
    cat_box.input = I(input + "_input");
    $("#" + input + "_box a").remove();
    $(cat_box.input).val("");
    $(cat_box.output).val("");
    $(cat_box.input).show().blur();
}

function focus_category_box(box) {
    input = box.id.replace("_box", "") + "_input";
    if (!cat_box.focus && I(input).style.display == 'block') I(input).focus();
}
...
function update_category() {
    ...
    $(cat_box.output).trigger("change");
    try {
        eval(cat_box.output.id + "_onUpdate(cat_box.input,cat_box.output);");
    } catch (e) {}
}

function select_cat_item(obj) // new
{
    cat_box.selected = [obj.id, obj.value];
    if (cat_box.multi) {
        var found = jQuery.inArray(obj.id, split_string(cat_box.output.value)) > -1;
        if (found) {
            $(cat_box.input).focus();
            return;
        }
        cat_box.output.value += obj.id + ",";
    } else {
        log(cat_box);
        cat_box.output.value = obj.id;
        cat_box.input.style.display = "none";
    }
    $(cat_box.output).attr("title", obj.value);
    if (cat_box.ctype == "first")
        show_cat = "<a inp='" + cat_box.output.id + "' onclick='remove_category(this," + obj.id + ")'>" + obj.value + "<b>x</b></a>"; //title=obj.value
    else
        show_cat = "<a inp='" + cat_box.output.id + "' onclick='remove_category(this," + obj.id + ")'><b>x</b>" + obj.value + "</a>"; //title=obj.value
    if (cat_box.container_id) {
        $("#" + cat_box.container_id).show().append(show_cat);
        $("#" + cat_box.container_id).css("display", "block");
    } else {
        $(cat_box.input).before(show_cat);
    }
    $(cat_box.input).val("");
    update_category();
}
...
function set_combo_Items(name, cat_list, no_evt) {
    $("#" + name + "_input").focus();
    cat_list = cat_list + "";
    single_cat = cat_list.split(",");
    var source_array;
    ////$("#"+name).val('');
    document.getElementById(name).value = "";
    if (typeof(cat_box.container_id) != 'undefined' && cat_box.container_id != '') {
        ////$("#"+cat_box.container_id).html('');
        document.getElementById(cat_box.container_id).innerHTML = "";
    }
    $("#" + name + "_box a").remove();
    cat_box.no_evt = no_evt;
    if (cat_list == "") {
        $(cat_box.input).show().blur();
        return;
    }
    if (name.indexOf("agent") >= 0) {
        source_array = agent_json_obj;
        index_id = "agentid";
        index_name = "agent_name";
    } else {
        source_array = cat_json_obj;
        index_id = "cat_id";
        index_name = "cat_name";
    }
    for (r = 0; r < single_cat.length; r++) {
        cat_json_obj_length = source_array.length;
        for (var i = 0; i < cat_json_obj_length; i++) {
            if (single_cat[r] == source_array[i][index_id]) {
                cat_obj = {};
                cat_obj.id = source_array[i][index_id];
                cat_obj.value = source_array[i][index_name];
                select_cat_item(cat_obj);
            }
        }
    }
    $("#" + name + "_input").blur();
...
function set_combo_value(name, value, a) {
...
function toggle_type(classname, p) {
    if (p == 1) {
        checked = true;
        checkbox_array = $("." + classname + ":not(:checked)");
    } else {
        checked = false;
        checkbox_array = $("." + classname + ":checked");
    }
    checkbox_array.each(function() {
        obj_li = $(this).parent();
        obj_li.trigger("mousedown");
    });
...
$(document).ready(function() {
    var combo_alert_html = '<table id="combo_alert" class="combo_body" style="margin: 0px; left: 797px; top: -528px;display:none;"><tbody><tr style="height: 27px;"><td class="tl"></td><td class="t"><label class="combo_alert_caption">Alert</label><i></i></td><td class="tr"></td></tr><tr><td class="ml"></td><td class="combo_data" style="width: 200px;" id="combo_msg"></td><td class="mr"></td></tr><tr><td class="bl"></td><td class="b"></td><td class="br"></td></tr></tbody></table>';
    setTimeout(function() {
        $('body').append(combo_alert_html);
    }, 10);
});

$(".advance_area_unit").live("change", function() {
    var combo_name = this.id.replace("_conv_unit", "");
    var unit = $("#" + combo_name + "_conv_unit").val();
    var html = $("#hidden_options_" + unit).html();
    $("#" + combo_name + "_combo_items").html(html);
    if (unit == 3) //marla
        $("#" + combo_name + "_unit_combo").show();
    else
        $("#" + combo_name + "_unit_combo").hide();
    setTimeout("if(combo) combo.load_options();", 10);
});

function get_ajax_suggs_city(key, cont) {
    res = [];
    /* data_index = "cat_json_obj_"+typing_lang;
    source_array = window[data_index]; */

    source_array_length = combo_source_array.length;
    lower_case_key = key.toLowerCase();

    for (var i = 0; i < source_array_length; i++) {
        auto_cat_name = combo_source_array[i].cat_name.toLowerCase();
        if (auto_cat_name.indexOf(lower_case_key) > -1)
            res.push({
                id: combo_source_array[i].cat_id,
                value: combo_source_array[i].cat_name,
                info: combo_source_array[i].cat_city
            });
    }
    cont(res);
    //log(source_array_length);
}

/*imzee_live("#category_city","change",function(){	
		enable_disable_cat_search_input($(this).val(),"cat_id_input");
		if($("#cat_id_box").find("a").length != 0)
			remove_category($("#cat_id_box").find("a").get(0),$("#cat_id").val());
		load_combo_data($(this).val());
		}
);*/

window.onload = function() {
    if (document.getElementById("category_city") != null) {
        document.getElementById("category_city").onchange = function() {
            if (document.getElementById("map_for_listing") != null) {
                comm_ask_location(this.value);
            }
            on_change_category_city(this.value);
        };
    }
};

function on_change_category_city(category_id) {
    ////enable_disable_cat_search_input($(this).val(),"cat_id_input");
    enable_disable_cat_search_input(category_id, "cat_id_input");
    var cat_id_box_obj = document.getElementById("cat_id_box");
    if (cat_id_box_obj != null) {
        if (cat_id_box_obj.getElementsByTagName("a").length != 0)
            remove_category(cat_id_box_obj.getElementsByTagName("a")[0], document.getElementById("cat_id").value);
    }
    /*//if($("#cat_id_box").find("a").length != 0)
    	remove_category($("#cat_id_box").find("a").get(0),$("#cat_id").val());//*/
    ////load_combo_data($(this).val());
    load_combo_data(category_id);
}

function enable_disable_cat_search_input(val, div_id) {
    div_id_obj = document.getElementById(div_id);
    if (val == "" || val == 0) {
        ////$("#"+div_id).attr('disabled', true);
        div_id_obj.setAttribute('disabled', true);
        ////$("#"+div_id).css("background-color","#FFFFFF");
        div_id_obj.style.backgroundColor = "#FFFFFF";
        ////$("#"+div_id).css("color","#B5B5B5");
        div_id_obj.style.color = "#B5B5B5";
        ////$("#"+div_id).val("Please select city first.");
        div_id_obj.value = "Please select city first.";
    } else {
        ////$("#"+div_id).attr('disabled', false);
        div_id_obj.removeAttribute('disabled');
        ////$("#"+div_id).css("color","");
        div_id_obj.style.color = "";
        ////$("#"+div_id).val("");
        div_id_obj.value = div_id_obj.defaultValue;
        $("#" + div_id).focus();
        $("#" + div_id).blur();
    }
}

function load_combo_data(city_id) {

    var file_id = city_id || $('#tab_city').val();
    window.combo_source_array = [];

    data_index = "cat_json_object_en";
    ...
    if (typeof(file_id) !== 'undefined') {
        ...
        if (typeof(window[data_index][file_id]) == 'undefined') {
            var cache_url = this_domain_property.replace(/\/property|\/newsite/, '') + "/zameen/js/locations/ajax_category_list_" + file_id + "_en.js";
...
/* imzee_live function*/
var imzee_live_events = [];

function imzee_live(selector) {
    //imzee_live_events.push(arguments);
    live_loaded = ($.fn.live == undefined) ? 0 : 1;
    var event, callback;
    for (var i = 1; i < arguments.length; i++) {
        ...
        if (i % 2 == 0 && i > 1) {
            if (live_loaded)
                ...
                $(selector).livequery(event, callback);
        }
    }
}
/* imzee_live function*/