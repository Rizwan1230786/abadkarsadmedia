                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <?php if (strpos($_SERVER['HTTP_ACCEPT'], 'image/webp') !== false) { ?>
                                    <td><img src="{{ is_null($develop->file) ? asset('assets/images/developer/webp/' . $develop->image_webp) : asset('assets/images/developer/' . $develop->image) }}" width="50px" height="50px"></td>
                                <?php } else { ?>
                                    <td><img src="{{ is_null($develop->file) ? asset('assets/images/developer/jpg/' . $develop->image) : asset('assets/images/developer/' . $develop->image) }}" width="50px" height="50px"></td>
                                <?php } ?>
                                <td>{{Str::limit($develop->address, 20)}}</td>
                                <td>{{ $develop->phone_no}}</td>
                                <td style="text-align: center;"><span class="m-badge  m-badge--wide">{{$develop->created_at}}</span>
                                </td>
                                <td>
                                    <ul class="icons-list">

                                        <a href="/admin/update/form/developer/{{$develop->id}}">
